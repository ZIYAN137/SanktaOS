...
use crate::println;
    ...
    fn test_mmio_translation() {
        ...
        if let Some(area) = area {
            assert!(area.area_type() == AreaType::KernelMmio);
            assert!(area.map_type() == MapType::Direct);
        }

        // 测试页表翻译
        let translated_paddr = ms.page_table().translate(Vaddr::from_usize(mmio_vaddr));
        assert!(translated_paddr.is_some());
    ...
    #[test_case]
    fn test_map_mmio_existing_mapping() {
        let mut ms = MemorySpace::new();
        ...
        // 应该返回相同的虚拟地址
        assert!(vaddr1.as_usize() == vaddr2.as_usize());
        println!(
    ...
    fn test_unmap_mmio_normal() {
        ...
        // 验证映射存在
        let vpn = Vpn::from_addr_floor(vaddr);
        assert!(ms.find_area(vpn).is_some());
    ...
    fn test_mmio_multiple_regions() {
        ...
        // 验证两个区域都存在
        assert!(ms.find_area(Vpn::from_addr_floor(vaddr1)).is_some());
        assert!(ms.find_area(Vpn::from_addr_floor(vaddr2)).is_some());

        // 取消映射第一个区域
        ms.unmap_mmio(vaddr1, REGION_SIZE)
            .expect("Failed to unmap region 1");
        assert!(ms.find_area(Vpn::from_addr_floor(vaddr1)).is_none());
        assert!(ms.find_area(Vpn::from_addr_floor(vaddr2)).is_some());

        // 取消映射第二个区域
        ms.unmap_mmio(vaddr2, REGION_SIZE)
            .expect("Failed to unmap region 2");
        assert!(ms.find_area(Vpn::from_addr_floor(vaddr2)).is_none());
    ...
    #[test_case]
    fn test_mprotect_multiple_areas() {
    ...
    #[test_case]
    fn test_mprotect_partial_front() {
        ...
        assert!(front_area.permission() == UniversalPTEFlag::user_read());
        println!("  Front area has R-only permission");

        // 验证后2页是读写
        let back_area = ms.find_area(Vpn::from_usize(0x7002)).unwrap();
        assert!(back_area.permission() == UniversalPTEFlag::user_rw());
    ...
    fn test_mprotect_partial_back() {
        ...
        assert!(front_area.permission() == UniversalPTEFlag::user_rw());
        println!("  Front area has RW permission");

        // 验证后2页是只读
        let back_area = ms.find_area(Vpn::from_usize(0x8002)).unwrap();
        assert!(back_area.permission() == UniversalPTEFlag::user_read());
    ...
    #[test_case]
    fn test_mprotect_partial_middle() {
    ...
    #[test_case]
    fn test_mprotect_partial_pte_flags() {
    ...
    #[test_case]
    fn test_mprotect_partial_single_page() {
