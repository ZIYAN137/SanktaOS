//! RISC-V 架构相关的启动代码
//!
//! 该模块包含主核/从核的启动与早期初始化流程（以 RISC-V virt 平台为主要目标）。
//! 具体分工大致为：
//! - `entry.S`：最早期的汇编入口，完成最小环境准备并跳入 Rust
//! - 本文件：主核初始化流程与从核唤醒/上线相关逻辑
//!
//! # 启动流程（概览）
//!
//! 典型顺序（以实现为准）：
//! 1. 早期汇编入口初始化最小运行环境
//! 2. 初始化 MM、Trap、设备与设备树等基础设施
//! 3. 在启用定时器中断前启动从核（多核场景）
//! 4. 创建并切入第一个任务（init/kthreadd 等），进入正常调度
//!
//! 说明：多核上线与调度唤醒可能依赖 IPI（见 `os/src/arch/riscv/ipi.rs`）。
