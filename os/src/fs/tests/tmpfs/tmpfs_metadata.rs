...
#[test_case]
fn test_tmpfs_metadata_initial() {
    ...
    assert!(metadata.mode.bits() & 0o777 == 0o644);
}
...
fn test_tmpfs_metadata_directory() {
    ...
    assert!(metadata.nlinks >= 2); // . 和至少父目录
    assert!(metadata.mode.bits() & 0o777 == 0o755);
}

#[test_case]
fn test_tmpfs_metadata_size_update() {
    let fs = create_test_tmpfs();
    let data = b"Hello, tmpfs!";
    let file = create_test_file_with_content(&fs, "test.txt", data).unwrap();

    let metadata = file.metadata().unwrap();
    assert!(metadata.size == data.len());

    // 追加数据
    let more_data = b" More data.";
    assert!(file.write_at(data.len(), more_data).is_ok());

    let metadata = file.metadata().unwrap();
    assert!(metadata.size == data.len() + more_data.len());
}

#[test_case]
fn test_tmpfs_metadata_truncate_update() {
    let fs = create_test_tmpfs();
    ...
    // 截断到更大
    assert!(file.truncate(100).is_ok());
    let metadata = file.metadata().unwrap();
    assert!(metadata.size == 100);

    // 截断到0
    assert!(file.truncate(0).is_ok());
    let metadata = file.metadata().unwrap();
    assert!(metadata.size == 0);
}

#[test_case]
fn test_tmpfs_metadata_mode() {
    let fs = create_test_tmpfs();
    let root = fs.root_inode();
...
fn test_tmpfs_metadata_timestamps_file() {
    ...
    // 时间戳应该非零
    assert!(ctime1.tv_sec > 0 || ctime1.tv_nsec > 0);

    // 写入数据（应更新 mtime）
    assert!(file.write_at(0, b"test data").is_ok());

    let meta2 = file.metadata().unwrap();
    // mtime 应该更新（或至少不变小）
    assert!(meta2.mtime.tv_sec >= mtime1.tv_sec);
}

#[test_case]
fn test_tmpfs_metadata_timestamps_dir() {
    let fs = create_test_tmpfs();
    let root = fs.root_inode();
    let dir = root
    ...
    assert!(meta1.ctime.tv_sec > 0 || meta1.ctime.tv_nsec > 0);
...
fn test_tmpfs_metadata_nlinks() {
    ...
    let subdir = dir
        ...
        .unwrap();
    let meta = dir.metadata().unwrap();
    assert!(meta.nlinks >= 3); // . 和父目录和子目录的 ..
}

#[test_case]
fn test_tmpfs_metadata_inode_number() {
    let fs = create_test_tmpfs();
    let root = fs.root_inode();
    ...
    assert!(meta1.inode_no != meta3.inode_no);
    assert!(meta2.inode_no != meta3.inode_no);
}

#[test_case]
fn test_tmpfs_metadata_after_unlink() {
    let fs = create_test_tmpfs();
    let root = fs.root_inode();
    let file = root
    ...
    // 删除前元数据应该有效
    let meta1 = file.metadata().unwrap();
    assert!(meta1.nlinks == 1);

    // 删除文件
    assert!(root.unlink("temp.txt").is_ok());
