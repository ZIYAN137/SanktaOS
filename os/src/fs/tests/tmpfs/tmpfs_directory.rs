...
#[test_case]
fn test_tmpfs_nested_directories() {
    ...
    assert!(&buf[..] == b"deep file");
}

#[test_case]
fn test_tmpfs_readdir_nested() {
    let fs = create_test_tmpfs();
    let root = fs.root_inode();
    ...
    assert!(entries.len() == 5); // ., .., file1.txt, subdir, file2.txt
...
#[test_case]
fn test_tmpfs_directory_not_empty() {
    let fs = create_test_tmpfs();
    let root = fs.root_inode();
...
fn test_tmpfs_parent_link() {
    ...
    assert!(parent.metadata().unwrap().inode_no == dir1.metadata().unwrap().inode_no);

    // 再往上一层
    let grandparent = parent.lookup("..").unwrap();
    assert!(grandparent.metadata().unwrap().inode_no == root.metadata().unwrap().inode_no);
}

#[test_case]
fn test_tmpfs_many_entries() {
    let fs = create_test_tmpfs();
    let root = fs.root_inode();
    ...
    assert!(entries.len() == 102); // 100 files + . + ..

    // 随机查找几个
    assert!(root.lookup("file_0.txt").is_ok());
    assert!(root.lookup("file_50.txt").is_ok());
    assert!(root.lookup("file_99.txt").is_ok());
}
