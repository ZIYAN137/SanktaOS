...
fn test_tmpfs_truncate() {
    ...
    // 验证初始大小
    assert!(inode.metadata().unwrap().size == 13);

    // 截断到 5 字节
    let result = inode.truncate(5);
    assert!(result.is_ok());
    assert!(inode.metadata().unwrap().size == 5);
...
fn test_tmpfs_unlink() {
    ...
    // 验证文件存在
    assert!(root.lookup("to_delete.txt").is_ok());
    ...
    // 验证文件不存在
    assert!(root.lookup("to_delete.txt").is_err());
}
...
fn test_tmpfs_rmdir() {
    ...
    // 验证目录不存在
    assert!(root.lookup("empty_dir").is_err());
}

#[test_case]
fn test_tmpfs_lookup_dot_dotdot() {
    let fs = create_test_tmpfs();
    let root = fs.root_inode();

    // 测试根目录的 "."
    let dot = root.lookup(".").unwrap();
    assert!(dot.metadata().unwrap().inode_no == root.metadata().unwrap().inode_no);

    // 测试根目录的 ".." (应该指向自己)
    let dotdot = root.lookup("..").unwrap();
    assert!(dotdot.metadata().unwrap().inode_no == root.metadata().unwrap().inode_no);
    ...
    assert!(parent.metadata().unwrap().inode_no == root.metadata().unwrap().inode_no);
}

#[test_case]
fn test_tmpfs_already_exists() {
    let fs = create_test_tmpfs();
    let root = fs.root_inode();
