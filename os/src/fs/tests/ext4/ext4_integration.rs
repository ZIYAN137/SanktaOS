...
#[test_case]
fn test_ext4_complete_workflow() {
    // 创建 Ext4 文件系统
...
fn test_ext4_mixed_operations() {
    ...
    let names: alloc::vec::Vec<_> = entries.iter().map(|e| e.name.as_str()).collect();
    assert!(names.contains(&"subfile1.txt"));
}

#[test_case]
fn test_ext4_nested_directory_structure() {
    // 创建多层嵌套目录结构
    let fs = create_test_ext4();
    let root = fs.root_inode();
    ...
    assert!(&buf[..] == b"Deep content");
}

#[test_case]
fn test_ext4_vfs_interface_completeness() {
    // 验证所有 VFS 接口方法都正常工作
    let fs = create_test_ext4();
    ...
    assert!(file.write_at(0, b"test").is_ok());
    let mut buf = vec![0u8; 4];
    assert!(file.read_at(0, &mut buf).is_ok());
    assert!(file.truncate(2).is_ok());
    ...
    let dir = root
        ...
        .unwrap();
    assert!(dir.metadata().is_ok());
    assert!(dir.readdir().is_ok());
    assert!(dir.lookup("test.txt").is_err()); // 不在这个目录
    assert!(
        dir.create("subfile.txt", FileMode::from_bits_truncate(0o644))
            .is_ok()
    );
    assert!(dir.lookup("subfile.txt").is_ok());

    // 清理
    assert!(root.unlink("test.txt").is_ok());
    assert!(dir.unlink("subfile.txt").is_ok());
    assert!(root.unlink("testdir").is_ok());
}

#[test_case]
fn test_ext4_filesystem_sync() {
    // 测试文件系统同步功能
    let fs = create_test_ext4();
    let root = fs.root_inode();
