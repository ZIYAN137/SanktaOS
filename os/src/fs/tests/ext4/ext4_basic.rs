...
fn test_ext4_write_at_offset() {
    ...
    inode.read_at(0, &mut buf).unwrap();
    assert!(&buf[..] == b"01234ABCDE");
}

#[test_case]
fn test_ext4_read_at_offset() {
    // 创建文件
    let fs = create_test_ext4();
    let inode = create_test_file_with_content(&fs, "test.txt", b"0123456789").unwrap();
...
#[test_case]
fn test_ext4_lookup_nonexistent() {
    // 创建 Ext4 文件系统
    let fs = create_test_ext4();
    let root = fs.root_inode();
...
#[test_case]
fn test_ext4_unlink_nonexistent() {
    // 创建 Ext4 文件系统
    let fs = create_test_ext4();
    let root = fs.root_inode();
...
fn test_ext4_read_beyond_eof() {
    ...
    let mut buf = vec![0u8; 10];
    let bytes_read = inode.read_at(0, &mut buf).unwrap();
    assert!(bytes_read <= 5); // 最多只能读取 5 字节
}

#[test_case]
fn test_ext4_empty_file() {
    // 创建空文件
    let fs = create_test_ext4();
