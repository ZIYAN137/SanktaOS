...
#[test_case]
fn test_sysfs_full_initialization() {
    let sysfs = create_test_sysfs_with_tree().unwrap();
    let root = sysfs.root_inode();

    // 验证所有顶层目录都存在
    assert!(root.lookup("class").is_ok());
    assert!(root.lookup("devices").is_ok());
    assert!(root.lookup("kernel").is_ok());
    assert!(root.lookup("block").is_ok()); // symlink
}

#[test_case]
fn test_sysfs_filesystem_type() {
    let sysfs = create_test_sysfs_with_tree().unwrap();
    assert!(sysfs.fs_type() == "sysfs");
}

#[test_case]
fn test_sysfs_sync_after_init() {
    let sysfs = create_test_sysfs_with_tree().unwrap();
    assert!(sysfs.sync().is_ok());
}

#[test_case]
fn test_sysfs_statfs_after_init() {
    let sysfs = create_test_sysfs_with_tree().unwrap();
...
#[test_case]
fn test_sysfs_multiple_init() {
    let sysfs = SysFS::new();
    assert!(sysfs.init_tree().is_ok());
...
#[test_case]
fn test_sysfs_readdir_all_entries() {
    let sysfs = create_test_sysfs_with_tree().unwrap();
    let root = sysfs.root_inode();
...
#[test_case]
fn test_sysfs_symlink_resolution() {
    let sysfs = create_test_sysfs_with_tree().unwrap();
    let root = sysfs.root_inode();
    ...
    assert!(target == "class/block");
}

#[test_case]
fn test_sysfs_concurrent_access() {
    let sysfs = create_test_sysfs_with_tree().unwrap();
    let root = sysfs.root_inode();
...
#[test_case]
fn test_sysfs_all_class_directories() {
    let sysfs = create_test_sysfs_with_tree().unwrap();
    let root = sysfs.root_inode();
    let class_dir = root.lookup("class").unwrap();
