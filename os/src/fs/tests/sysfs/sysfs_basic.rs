...
#[test_case]
fn test_sysfs_root_empty_initially() {
    let sysfs = create_test_sysfs();
    let root = sysfs.root_inode();
...
#[test_case]
fn test_sysfs_root_is_directory() {
    let sysfs = create_test_sysfs();
    let root = sysfs.root_inode();
...
#[test_case]
fn test_sysfs_multiple_instances() {
    let sysfs1 = create_test_sysfs();
    let sysfs2 = create_test_sysfs();

    assert!(sysfs1.fs_type() == "sysfs");
    assert!(sysfs2.fs_type() == "sysfs");
