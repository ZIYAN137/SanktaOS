...
mod tests {
    ...
    fn test_current_cpu() {
        ...
        let _guard = PreemptGuard::new();
        let cpu = current_cpu();
        assert!(cpu.cpu_id < unsafe { NUM_CPU });
    }
    ...
    fn test_cpu_of() {
        let cpu0 = cpu_of(0);
        assert!(cpu0.cpu_id == 0);

        let num_cpu = unsafe { NUM_CPU };
        if num_cpu > 1 {
            let cpu1 = cpu_of(1);
            assert!(cpu1.cpu_id == 1);
        }
    }

    /// 测试 PerCpu 数据独立性（多核场景）
    #[test_case]
    fn test_per_cpu_independence() {
