...
mod tests {
    ...
    // 基础：新增后应存在于队列，队列非空
    #[test_case]
    fn test_task_queue_add_and_contains() {
    ...
    // FIFO 顺序：先入先出
    #[test_case]
    fn test_task_queue_pop_order_fifo() {
        let mut q = TaskQueue::new();
        let t1 = mk_task(10);
        let t2 = mk_task(11);
        q.add_task(t1.clone());
    ...
    fn test_task_queue_contains_arc_identity() {
        ...
        q.add_task(t1.clone());
        assert!(q.contains(&t1));
        assert!(q.contains(&t1_clone));
        assert!(!q.contains(&t1_other));
    }

    // 空队列状态切换
    #[test_case]
    fn test_task_queue_empty_state() {
        let mut q = TaskQueue::new();
        assert!(q.is_empty());
        let t = mk_task(40);
        q.add_task(t.clone());
