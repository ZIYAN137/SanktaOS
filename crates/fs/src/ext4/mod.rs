...
impl Ext4FileSystem {
    ...
    pub fn open(
        ...
        log::info!("[Ext4] Opening Ext4 filesystem on block device");
        log::info!(
        ...
        log::info!("[Ext4] Calling ext4_rs::Ext4::open...");
        let ext4 = ext4_rs::Ext4::open(adapter);
        log::info!("[Ext4] ext4_rs returned successfully");
        ...
        log::info!("[Ext4] Filesystem opened successfully");
        Ok(fs)
    }
}
...
fn validate_superblock(adapter: &BlockDeviceAdapter) -> Result<(), FsError> {
    ...
    if buf.len() < MAGIC_OFFSET + 2 {
        log::error!("[Ext4] Superblock read too small: {}", buf.len());
        return Err(FsError::IoError);
    }
    let magic = u16::from_le_bytes([buf[MAGIC_OFFSET], buf[MAGIC_OFFSET + 1]]);
    if magic != EXT4_MAGIC {
        log::error!("[Ext4] Invalid ext4 magic: {:#x}", magic);
    ...
    if blocks_per_group == 0 {
        log::error!("[Ext4] Invalid superblock: blocks_per_group=0");
        return Err(FsError::InvalidArgument);
    }
    Ok(())
}
