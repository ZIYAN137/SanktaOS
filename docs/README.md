...
本目录存放 SanktaOS 项目的设计文档、模块说明与写作指南。此 README 面向"文档作者与维护者"，用于解释组织结构、写作约定、如何预览/发布。
...
- 风格与结构
  ...
  - 代码片段应最小可读，可附运行/调用路径。
- 校验与工具
  - 风格与链接检查可参考：`docs/scripts/style-check.md`、`docs/scripts/rewrite_links.md`
  - 提交前自查：新增文档是否需要出现在 SUMMARY.md；是否在对应子目录 README 中被导航到。

## 新增或更新文档的建议流程

1. 在对应子目录内新增 Markdown 文件，或同步更新该子目录 README 的导航。
2. 若需要在 mdBook 中展示，更新 `docs/SUMMARY.md`，保持目录结构清晰。
3. 本地预览（如使用 mdBook）：`mdbook serve -n 0.0.0.0 -p 4000`，浏览器打开：`$BROWSER http://127.0.0.1:4000/`
4. 在 PR 描述中简述新增内容与对应源码位置，便于评审。

## 常见问题

- 为什么我新增的文档在左侧目录看不到？
  - 需要把文档添加到 `docs/SUMMARY.md` 中。README 只提供作者指引，不参与 mdBook 的目录生成。
- 链接在 mdBook 中 404？
  - 检查相对路径是否以 `docs/` 为根进行组织；必要时使用脚本 `docs/scripts/rewrite_links.md` 的建议重写规则。
